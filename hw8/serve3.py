...
def handle_connection(sock):
	while 1:
		try:
			...
			#check for POST and grab the Content-Length integer
			tmp = data
			tmp.strip()
			allLines = tmp.split("\r\n")
...
def handle_request(request):
	...
	data = app(environ, faux_start_response)

	response = ""
	response += "HTTP/1.1 %s\r\n" % (_status)
	response += "Date: %s \r\n" % datetime.datetime.now().strftime("%a, %d %b %Y %H:%M:%S EST")
	response += "Server: WSGIServer/0.1 Python/2.7\r\n"
	for k,v in _headers:
		if k == 'Content-type':
